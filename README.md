...
- 想把文字做成**手写外观的图片**（卡片、信件、笔记样式图、手写风海报）
- 要求笔画**出自本人的手写**，不接受套用某个手写字体
- 不愿把字迹上传到任何在线服务 —— **纯本地运行**是硬需求
- 要求**不使用 AI 模型**、结果确定可复现（同一 seed 渲染两次逐字节一致）
- 主要处理**中文汉字**（字形库按汉字设计，这是主线）

**不适合**：

- 想用机器学习生成**任意人**的手写风格 —— 那属于 RNN / 深度学习路线
- 不想先写一张字版、希望给段文字就出图 —— 字体扰动类工具门槛更低
- 需要图形界面、实时预览、框选编辑或 PDF 导出 —— 本项目只有命令行
- 目的是**伪造他人签名或笔迹** —— 明确禁止，见文末免责声明
